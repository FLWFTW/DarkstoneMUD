...
void do_mpmset(CHAR_DATA *ch, const char* argument)
{
	...
	if ( !IS_NPC( ch ) || IS_AFFECTED(ch, AFF_CHARM) || ch->GetConnection() )
	{
	...
	if ( !str_cmp( arg2, "scent" ) )
	{
		if ( value < 0 || ScentController::instance()->findScent(value) == NULL )
		{
			progbug("MpMset: Invalid scent ID", ch);
			return;
		}
	...
	if ( !str_cmp( arg2, "mentalstate" ) )
	{
		...
		if ( arg3[0] == '-' ) {
			value = atoi(++ptr);
			victim->mental_state = UMAX(-100, victim->mental_state - value);
		} else if ( arg3[0] == '+' ) {
			value = atoi(++ptr);
			victim->mental_state = UMIN(100, victim->mental_state + value);
		} else {        
	...
	if ( !str_cmp( arg2, "recall" ) )
	{
		if ( IS_NPC( victim ) )
		{
			progbug("MpMset: mob can't set recall", ch);
			return;
		}
		if ( get_room_index( value ) == NULL )
		{
			progbug("MpMset: Invalid recall vnum", ch);
			return;
		}
...
void do_mposet(CHAR_DATA *ch, const char* argument)
{
	...
	if ( !str_cmp( arg2, "level" ) )
	{
		...
		obj->level = value;
#else
		send_to_char("Use of this value is depreciated.\n", ch);
#endif       
		return;
	}
	...
	if ( !str_cmp( arg2, "short" ) )
	{
		 ...
		 */
		if ( str_infix( "mprename", obj->name_.c_str() ) )
		{
			sprintf( buf, "%s %s", obj->name_.c_str(), "mprename" );
			obj->name_ = buf;
		}
		return;
	}
