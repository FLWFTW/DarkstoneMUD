...
void do_reset(Character * ch, const char* argument);
void do_rreset(Character * ch, const char* argument);

/* Externals */
extern	int	top_reset;
char *		sprint_reset	 ( CHAR_DATA *ch, RESET_DATA *pReset,
					sh_int num, bool rlist );
RESET_DATA *	parse_reset	 ( AREA_DATA *tarea, const char *argument,
					CHAR_DATA *ch );
...
bool is_room_reset   ( RESET_DATA *pReset, ROOM_INDEX_DATA *aRoom,
                            AREA_DATA *pArea );
void add_obj_reset   ( AREA_DATA *pArea, char cm, OBJ_DATA *obj,
                            int v2, int v3 );
void delete_reset    ( AREA_DATA *pArea, RESET_DATA *pReset ) ;
void instaroom       ( AREA_DATA *pArea, ROOM_INDEX_DATA *pRoom,
			    bool dodoors );
#define RID ROOM_INDEX_DATA
RID *find_room       ( CHAR_DATA *ch, const char *argument,
                            ROOM_INDEX_DATA *pRoom );
#undef RID
void edit_reset      ( CHAR_DATA *ch, const char *argument, AREA_DATA *pArea,
                            ROOM_INDEX_DATA *aRoom );
#define RD RESET_DATA
RD *find_reset       ( AREA_DATA *pArea, ROOM_INDEX_DATA *pRoom,
			    int num );
#undef RD
...
ROOM_INDEX_DATA *find_room( CHAR_DATA *ch, const char *argument,
                            ROOM_INDEX_DATA *pRoom )
{
  ...
  if ( arg[0] == '\0' )
    ...
    pRoom = get_room_index(dotted_to_vnum(ch->GetInRoom()->vnum, arg));
  if ( !pRoom )
  {
	...
	else
	{
		...
		for ( reset = pArea->first_reset; reset; reset = reset->next )
		{
			...
			if ( (pObjTo = get_obj_index(reset->arg1)) &&
					is_name(arg, pObjTo->name_.c_str()) && ++cnt == num )
				break;
		}
		if ( !pObjTo || !reset )
		{
	...
	else
	{
		...
		for ( reset = pArea->first_reset; reset; reset = reset->next )
		{
			...
			if ( (pMob = get_mob_index(reset->arg1)) &&
					is_name(arg, pMob->playerName_.c_str()) && ++cnt == num )
				break;
		}
		if ( !pMob || !reset )
		{
		 ...
		 ROOM_INDEX_DATA *aRoom )
{
	...
	if ( !*arg || !str_cmp(arg, "?") )
	{
		char *nm = (ch->substate == SUB_REPEATCMD ? (char *) "" : (aRoom ? (char *) "rreset "
					: (char *) "reset "));
	...
	if ( !str_prefix( arg, "mobile" ) )
	{
		...
		if ( !(pMob = get_mob_index(dotted_to_vnum(ch->GetInRoom()->vnum, arg))))
		{
	...
	if ( !str_prefix(arg, "object") )
	{
		...
		if ( !(pObj = get_obj_index(dotted_to_vnum(ch->GetInRoom()->vnum, arg))))
		{
		...
		if ( !str_prefix( arg, "put" ) )
		{
			...
			argument = one_argument(argument, arg);
			if ( (vnum = dotted_to_vnum(ch->GetInRoom()->vnum, arg)) < 1 )
				vnum = 1;
	...
	if ( !str_cmp(arg, "trap") )
	{
		...
		{
			vnum = (aRoom ? aRoom->vnum : ch->GetInRoom()->vnum);
			extra = TRAP_ROOM;
		}
		else
		{
			...
			{
				vnum = dotted_to_vnum(ch->GetInRoom()->vnum, oname);
...
void do_reset(CHAR_DATA *ch, const char* argument)
{
	...
	if ( ch->substate == SUB_REPEATCMD )
	{
		...
		if ( pArea && pArea != ch->pcdata->area && pArea != ch->GetInRoom()->area )
		{
			AREA_DATA *tmp;
	...
	if ( !pArea && get_trust(ch) > LEVEL_STONE_ADEPT )
	{
		char fname[80];
...
void do_rreset(CHAR_DATA *ch, const char* argument)
{
	ROOM_INDEX_DATA *pRoom;
	...
	if ( ch->substate == SUB_REPEATCMD )
	{
		...
		if ( !pRoom )
		{
			...
			bug("do_rreset: %s's dest_buf points to invalid room", ch->getName().c_str());
		}
		ch->substate = SUB_NONE;
		ch->dest_buf = NULL;
		return;
...
void do_instaroom(CHAR_DATA *ch, const char* argument)
{
  ...
  if ( IS_NPC(ch) || get_trust(ch) < LEVEL_ENGINEER || !ch->pcdata ||
      !ch->pcdata->area )
  {
  ...
  if ( !can_rmodify(ch, pRoom) )
    return;
  if ( pRoom->area != pArea && get_trust(ch) < LEVEL_STONE_ADEPT )
  {
	...
	for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
	{
		switch( pReset->command )
		{
			...
			case 'M':
				...
				{
					bug( "Reset_area: 'M': bad mob vnum %s.", vnum_to_dotted(pReset->arg1) );
					continue;
				}
				if ( !(pRoomIndex = get_room_index(pReset->arg3)) )
				{
					bug( "Reset_area: 'M': bad room vnum %s.", vnum_to_dotted(pReset->arg3) );
					continue;
				}
				...
				level = URANGE(0, mob->level - 2, LEVEL_HERO_MAX);
				break;
			...
			case 'E':
				...
				{
					bug( "Reset_area: 'E' or 'G': bad obj vnum %s.", vnum_to_dotted(pReset->arg1) );
					continue;
				}

				if ( pObjIndex->total_count >= pObjIndex->rare && pObjIndex->rare >0 ) {
					obj = NULL;
					lastobj = NULL;
					break;
				}
				...
				obj->level = URANGE(0, obj->level, LEVEL_HERO_MAX);
#endif       
			...
			case 'O':
				if ( !(pObjIndex = get_obj_index(pReset->arg1)) )
				{
					bug( "Reset_area: 'O': bad obj vnum %s.", vnum_to_dotted(pReset->arg1) );
					continue;
				}
				if ( !(pRoomIndex = get_room_index(pReset->arg3)) )
				{
					bug( "Reset_area: 'O': bad room vnum %s.", vnum_to_dotted(pReset->arg3) );
					continue;
				}
				if ( pObjIndex->total_count >= pObjIndex->rare && pObjIndex->rare >0 ) {
					obj = NULL;
...
#ifdef USE_OBJECT_LEVELS
				obj->level = UMIN(obj->level, LEVEL_HERO_MAX);
#endif       
				obj->cost = 0;
			...
			case 'P':
				if ( !(pObjIndex = get_obj_index(pReset->arg1)) )
				{
					bug( "Reset_area: 'P': bad obj vnum %s.", vnum_to_dotted(pReset->arg1) );
					continue;
				}
				...
				if ( pReset->arg3 > 0 )
				{
					...
					{
						bug( "Reset_area: 'P': bad objto vnum %s.", vnum_to_dotted(pReset->arg3) );
						continue;
					}
				...
				else
				{
					...
					for ( iNest = 0; iNest < pReset->extra; iNest++ )
						...
						{
							bug( "Reset_area: 'P': Invalid nesting obj %s.", vnum_to_dotted(pReset->arg1) );
							iNest = -1;
							break;
						}
					if ( iNest < 0 )
...
#else
				obj = create_object(pObjIndex, number_fuzzy(UMAX(generate_itemlevel(pArea, pObjIndex),0)));
#endif       
			...
			case 'T':
				if ( IS_SET(pReset->extra, TRAP_OBJ) )
				{
					...
					if ( pReset->arg3 > 0 )
					{
						if ( !(pObjToIndex = get_obj_index(pReset->arg3)) )
						{
							bug( "Reset_area: 'T': bad objto vnum %s.", vnum_to_dotted(pReset->arg3) );
							continue;
						}
						if ( pArea->nplayer > 0 ||
								!(to_obj = get_obj_type(pObjToIndex)) ||
								(to_obj->GetCarriedBy() && !IS_NPC(to_obj->GetCarriedBy())) ||
								is_trapped(to_obj) )
							break;
					}
					else
			...
			case 'H':
				if ( pReset->arg1 > 0 )
				{
					...
					{
						bug( "Reset_area: 'H': bad objto vnum %s.", vnum_to_dotted(pReset->arg1) );
						continue;
					}
					if ( pArea->nplayer > 0 ||
							!(to_obj = get_obj_type(pObjToIndex)) ||
							!to_obj->GetInRoom() ||
							to_obj->GetInRoom()->area != pArea ||
			...
			case 'B':
				switch(pReset->arg2 & BIT_RESET_TYPE_MASK)
				{
					case BIT_RESET_DOOR:
						{
							...
							{
								bug( "Reset_area: 'B': door: bad room vnum %s.", vnum_to_dotted(pReset->arg1) );
								continue;
							}
					...
					case BIT_RESET_ROOM:
						if ( !(pRoomIndex = get_room_index(pReset->arg1)) )
						{
							bug( "Reset_area: 'B': room: bad room vnum %s.", vnum_to_dotted(pReset->arg1) );
							continue;
						}
					...
					case BIT_RESET_OBJECT:
						if ( pReset->arg1 > 0 )
						{
							if ( !(pObjToIndex = get_obj_index(pReset->arg1)) )
							{
								bug( "Reset_area: 'B': object: bad objto vnum %s.", vnum_to_dotted(pReset->arg1) );
								continue;
							}
			...
			case 'D':
				if ( !(pRoomIndex = get_room_index(pReset->arg1)) )
				{
					bug( "Reset_area: 'D': bad room vnum %s.", vnum_to_dotted(pReset->arg1) );
					continue;
				}
			...
			case 'R':
				if ( !(pRoomIndex = get_room_index(pReset->arg1)) )
				{
					bug( "Reset_area: 'R': bad room vnum %s.", vnum_to_dotted(pReset->arg1) );
					continue;
				}
		...
		switch( pReset->command )
		{
			...
			case 'M':
				if ( !(mob = get_mob_index(pReset->arg1)) )
					...
					mname = mob->playerName_.c_str();
				if ( !(room = get_room_index(pReset->arg3)) )
					rname = "Room: *BAD VNUM*";
				else
					rname = room->name_.c_str();
			...
			case 'P':
				...
				if ( pReset->arg3 > 0 )
				{
					...
					rname = (obj2 ? obj2->name_.c_str() : "Object2: *BAD VNUM*");
					lastobj = obj2;
				}
				else if ( !lastobj )
