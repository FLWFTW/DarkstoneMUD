     ...
     CHAR_DATA *ch, int op_num, bool or_sw, int dis_num)
{
  ...
  char Class[] = "MCTWVDRAGBK";                /* convert class to text     */
  char race[][3] =                          /* convert race to text      */
  {"Hu", "El", "Dw", "Ha", "Px", "Va", "Og", "HO", "HT", "HE", "Gi", "Gl", "Gn", "Ka", "Ar", "Ha", "Ey", "Mi", "EL", "Tr"};
  char clan[][4] = {
  "   ", "DS ", "VAM", "MAG", "   ", "   ", "   ", "   ","   ", "   ", "   "};
  char council[][4] =
  {"   ", "   ", "   ", "   ", "   ", "   ", "   ", "   ", "   ", "   "};
  ...
  while ( !feof(fp) )                       /* read each input record    */
  {
     ...
     if ( res )                             /* record is a match         */
     {
        ...
        {
           ch_printf(ch,
           "\n\r%-12s %-2s %1s %-2s %1s %3s %3s %5s %11s %-6s %s\n\r",
        ...
        if ( tot_match <= dis_num )         /* print record if applicable */
           ch_printf(ch,
  "%-12s %2hd %c %2s %c %3s %3s %5hd %11ld %6lu %s\n\r",
...
void do_diagnose(CHAR_DATA *ch, const char* argument)
{
	...
	if (!str_cmp(arg1, "mm")) {
		...
		if ( victim == ch )
		{
			...
			for ( itor = gTheWorld->GetBeginConnection(); itor != gTheWorld->GetEndConnection(); itor++ )
			{
				...
				//if ( d->snoop_by == ch->GetConnection() )
					//d->snoop_by = NULL;
			}
			return;
		}

		//if ( victim->GetConnection()->snoop_by )
		//{
			//send_to_char( "Busy.\n\r", ch );
			//return;
		//}
		...
		//victim->Connection->snoop_by = ch->Connection;
		send_to_char( "Ok.\n\r", ch );
		return;
	}
...
#define ZERO_MAX   1500
	...
	if (!str_cmp(arg1, "ob"))
	{
		...
		ch_printf(ch, "CHAR DATA "
			...
			ch->getName().c_str(), ch->affected_by, ch->perm_str, ch->mod_str);
		...
		{
			if ( !po->GetCarriedBy() && !po->GetInObj() ) continue;
			if ( !po->GetCarriedBy() )
			{
				pt = po;
				while( pt->GetInObj() ) 		  /* could be in a container on ground */
					pt=pt->GetInObj();
			}
			if ( ch==po->GetCarriedBy() || ch==pt->GetCarriedBy() )
			{
				px = po->pIndexData;
				ch_printf(ch,
					"\n\rINDEX_DATA vnum=%d name=%s level=%d extra_flags=%d\n\r",
					px->vnum, px->name_.c_str(), px->level, px->extra_flags);
				...
				ch_printf(ch,
					...
					po->name_.c_str(), po->wear_flags, po->wear_loc);
#endif
			...
			if ( mob_index_hash[cou] )
				for (pm = mob_index_hash[cou]; pm; pm = pm->next)
				{
					if ( pm->vnum >= vnum1 && pm->vnum <= vnum2
						...
						pager_printf( ch, "%5d %s\n\r", pm->vnum, pm->playerName_.c_str() );
				}
		}
		return;
	}
