...
void do_west(Character * ch, const char* argument);
void do_east(Character * ch, const char* argument);
void do_north(Character * ch, const char* argument);
void do_south(Character * ch, const char* argument);
...
char get_map_code( RID *room, int mode )
{
   ...
   switch(mode)
   {
      case 11: return count_lines(room->description_.c_str());
...
void draw_map (CD * ch, RID * rm, int flag, int mode) {
	...
	for (x = 0; x < 78; x++) {
	    ...
	    if(  (flag==1) )
	    {
	       if(map_index->map_of_vnums[y][x]==ch->GetInRoom()->vnum)
	       {
...
void do_lookmap(CD * ch, const char* argument) {
	...
	if( ! argument ) { 
	   mode = 0;
        ...
        sprintf( buf , ".------[Map %5.5d]-----------------------------------------------------------.\n\r",
			ch ->GetInRoom()-> map -> vnum);
	send_to_char (buf , ch);
	draw_map (ch, ch ->GetInRoom(), 1, mode);
...
void do_mapout(CD * ch, const char* argument) {
	char    buf[MAX_STRING_LENGTH];
	...
	RID     *this_rm, *tmp_r;         /* room ch is standing in */
	XD      *tmp_x;                   /* exit data */
	char    code;
	int     rooms,                    
	...
	if (!str_cmp (arg, "continue")) {
		if ( ch->prev_cmd == do_north )
		{
			...
			tmp = NULL;
			tmp_x = get_exit(ch->GetInRoom(), DIR_SOUTH );
			if ( tmp_x )
				tmp_r = tmp_x->to_room;
			if ( tmp_r )
				tmp = tmp_r->map;
		...
		if ( ch->prev_cmd == do_east )
		{
			...
			tmp = NULL;
			tmp_x = get_exit( ch->GetInRoom(), DIR_WEST );
			if ( tmp_x )
				tmp_r = tmp_x->to_room;
			if ( tmp_r )
				tmp = tmp_r->map;
		...
		if ( ch->prev_cmd == do_south )
		{
			...
			tmp = NULL;
			tmp_x = get_exit( ch->GetInRoom(), DIR_NORTH );
			if ( tmp_x )
				tmp_r = tmp_x->to_room;
			if ( tmp_r )
				tmp = tmp_r->map;
		...
		if ( ch->prev_cmd == do_west )
		{
			...
			tmp = NULL;
			tmp_x = get_exit( ch->GetInRoom(), DIR_EAST );
			if ( tmp_x )
				tmp_r = tmp_x->to_room;
			if ( tmp_r )
				tmp = tmp_r->map;
	...
	if (!str_cmp (arg, "addroom")) {
		...
		map->y		 = y;
		map->entry	 = code;
		ch->GetInRoom()->map = map;

		map_index->map_of_vnums[y][x] = ch->GetInRoom()->vnum;
...
add_new_room_to_map(CD *ch,MID *map,int row,int col,int proto_room,char code) {
	...
	if (get_room_index (i) == NULL) {
	    ...
	    location -> name_ = rm -> name_;
	    location -> description_ = rm -> description_;
...
const char   *const standard_room_names[] =
{
